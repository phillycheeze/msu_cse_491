...
__global__
void matPDEKernel(double alpha, double* E, double* E_prev, int n)
{
  int row = blockIdx.y*blockDim.y+threadIdx.y+1;
  int col = blockIdx.x*blockDim.x+threadIdx.x+1;
	       ...
	       const double M1,const double  M2, const double b)
{
    ...
    boundaryKernel<<<DimGrid, DimBlock>>>(E_prev, m, n);
    cudaThreadSynchronize();

    matPDEKernel<<<DimGrid, DimBlock>>>(alpha, E, E_prev, n);
    cudaThreadSynchronize();

    vecODEKernel<<<DimGrid, DimBlock>>>(R, E, epsilon, M1, M2, dt, kk, a, b, n);
    cudaThreadSynchronize();
}
