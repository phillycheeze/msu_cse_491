...
void matAllKernel(double alpha, double* E, double* E_prev, double* R, int n, int m, double epsilon, double M1, double M2, double dt, double kk, double a, double b)
{
  ...
  int tile_row = by * (TILE_DIM+2);
  int tile_col = bx * (TILE_DIM+2);

 // E_prev[tile_row + ty][tile_col + tx]
  tile_Eprev[ty][tx] = E_prev[(tile_row + ty)*(n+2) + tile_col + tx];
  ...
  if(by == 0 || by == TILE_DIM + 1) {
    tile_Eprev[by][0] = tile_Eprev[by][2];
    tile_Eprev[by][TILE_DIM+1] = tile_Eprev[by][TILE_DIM-1];
  }
  if(bx == 0 || bx == TILE_DIM + 1) {
    tile_Eprev[0][bx] = tile_Eprev[2][bx];
    tile_Eprev[TILE_DIM+1][bx] = tile_Eprev[TILE_DIM-1][bx];
  }
  ...
  if((row < n) && (col < n)) {
    row = row * (n+2);
    if((t_row < TILE_DIM) && (t_col < TILE_DIM))
      E[row + col] = tile_Eprev[t_row][t_col]+alpha*(tile_Eprev[t_row][t_col + 1]+tile_Eprev[t_row][t_col - 1]-4*tile_Eprev[t_row][t_col]+tile_Eprev[t_row+1][t_col]+tile_Eprev[t_row-1][t_col]);

    //ODE part, making optimizations
    e = E[row+col];
    r = R[row+col];
